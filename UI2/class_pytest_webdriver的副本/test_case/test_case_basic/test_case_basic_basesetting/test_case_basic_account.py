...
from page_objeck.Basic.basicsetting.basicsetting_account_page import Basic_Setting_Account
from page_objeck.Basic.basicsetting.basicsetting_customer_page import *
...
class TestCaseAccount():
    ...
    #  基础设置-新增账户
    @allure.epic("基础设置模块")
    @allure.feature("基础资料-账户模块")
    @allure.story("新增账户")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/account/add.yaml'))
    def test_2_add_account(self, udata, init_driver):
        '''基础设置-新增账户用例'''
        self.lp = Basic_Setting_Account(init_driver).add_Account(udata['customerName'])
    ...
    @allure.feature("基础资料-账户模块")
    @allure.story("查询账户")
    @pytest.mark.parametrize('udata', loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/account/query.yaml'))
    def test_3_seach_account(self, udata, init_driver):
        '''基础设置-新增账户用例'''
        self.lp = Basic_Setting_Account(init_driver).seach_Account(udata['customerName'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
    ...
    @allure.feature("基础资料-账户模块")
    @allure.story("修改账户名称")
    @pytest.mark.parametrize('udata', loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/account/change.yaml'))
    def test_4_change_customer(self, udata, init_driver):
        '''基础设置-新增账户用例'''
        self.lp = Basic_Setting_Account(init_driver).change_Account(udata['customerName'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
    ...
    @allure.story("删除单个账户")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/account/delete.yaml'))
    def test_5_delete_customer(self, udata, init_driver):
        '''基础设置-删除账户用例'''
        self.lp = Basic_Setting_Account(init_driver).delete_Account(udata['customerName'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
...
if __name__ == '__main__':
    ...
    pytest.main(
        ['-s', 'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/test_case/test_case_basic/test_case_basic_basesetting/test_case_basic_account.py',
