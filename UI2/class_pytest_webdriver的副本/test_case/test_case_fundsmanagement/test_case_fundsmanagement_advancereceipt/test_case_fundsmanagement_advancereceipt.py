...
from page_objeck.Fundsmanagement.Advancereceipt.fundsmanagement_advancereceipt_page import *
...
@pytest.mark.usefixtures('AdvanceReceipt')
class TestCaseadvancereceipt():

    #  基础设置-新增预收款单
    @allure.epic("资金管理")
    @allure.feature("预收款单模块")
    @allure.story("新增预收款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/advance receipt/add.yaml'))
    def test_2_add_advancereceipt(self, udata, init_driver):
        '''基础设置-新增预收款单用例'''
        self.lp = Fund_Management_advancereceipt(init_driver).add_advancereceipt(udata['Name'])
    ...
    @allure.epic("资金管理")
    @allure.feature("预收款单模块")
    @allure.story("修改预收款单名称")
    @pytest.mark.parametrize('udata', loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/advance receipt/change.yaml'))
    def test_4_change_customer(self, udata, init_driver):
        '''基础设置-新增预收款单用例'''
        self.lp = Fund_Management_advancereceipt(init_driver).change_advancereceipt(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
    ...
    @allure.epic("资金管理")
    @allure.feature("预收款单模块")
    @allure.story("删除单个预收款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/advance receipt/delete.yaml'))
    def test_5_delete_customer(self, udata, init_driver):
        '''基础设置-删除预收款单用例'''
        self.lp = Fund_Management_advancereceipt(init_driver).delete_advancereceipt(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
...
if __name__ == '__main__':
    ...
    pytest.main(
        ['-s', 'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/test_case/test_case_fundsmanagement/test_case_fundsmanagement_advancereceipt/test_case_fundsmanagement_advancereceipt.py',
