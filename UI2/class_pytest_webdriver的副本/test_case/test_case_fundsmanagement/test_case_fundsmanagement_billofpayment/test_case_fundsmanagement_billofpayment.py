...
from page_objeck.Fundsmanagement.Billofpayment.fundsmanagement_billofpayment_page import *
# from delete.login_page import *
from config.yamlload import loadyaml
import allure


@pytest.mark.usefixtures('Billofpayment')
class TestCasebillofpayment():

    #  基础设置-新增付款单
    @allure.epic("资金管理")
    @allure.feature("付款单模块")
    @allure.story("新增付款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/bill of payment/add.yaml'))
    def test_2_add_billofpayment(self, udata, init_driver):
        '''基础设置-新增付款单用例'''
        self.lp = Fund_Management_billofpayment(init_driver).add_billofpayment(udata['Name'])
    ...
    @allure.epic("资金管理")
    @allure.feature("付款单模块")
    @allure.story("修改付款单名称")
    @pytest.mark.parametrize('udata', loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/bill of payment/change.yaml'))
    def test_4_change_customer(self, udata, init_driver):
        '''基础设置-新增付款单用例'''
        self.lp = Fund_Management_billofpayment(init_driver).change_billofpayment(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
    ...
    @allure.epic("资金管理")
    @allure.feature("付款单模块")
    @allure.story("删除单个付款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/bill of payment/delete.yaml'))
    def test_5_delete_customer(self, udata, init_driver):
        '''基础设置-删除付款单用例'''
        self.lp = Fund_Management_billofpayment(init_driver).delete_billofpayment(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
...
if __name__ == '__main__':
    ...
    pytest.main(
        ['-s', 'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/test_case/test_case_fundsmanagement/test_case_fundsmanagement_billofpayment/test_case_fundsmanagement_billofpayment.py',
