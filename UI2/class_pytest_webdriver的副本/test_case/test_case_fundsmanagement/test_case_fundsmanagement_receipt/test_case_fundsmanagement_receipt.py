...
from page_objeck.Fundsmanagement.receipt.fundsmanagement_receipt_page import *
# from delete.login_page import *
from config.yamlload import loadyaml
import allure


@pytest.mark.usefixtures('Receipt')
class TestCasereceipt():

    #  基础设置-新增收款单
    @allure.epic("资金管理")
    @allure.feature("收款单模块")
    @allure.story("新增收款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/receipt/add.yaml'))
    def test_2_add_receipt(self, udata, init_driver):
        '''基础设置-新增收款单用例'''
        self.lp = Fund_Management_receipt(init_driver).add_receipt(udata['Name'])
    ...
    @allure.epic("资金管理")
    @allure.feature("收款单模块")
    @allure.story("修改收款单名称")
    @pytest.mark.parametrize('udata', loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/receipt/change.yaml'))
    def test_4_change_customer(self, udata, init_driver):
        '''基础设置-新增收款单用例'''
        self.lp = Fund_Management_receipt(init_driver).change_receipt(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
    ...
    @allure.epic("资金管理")
    @allure.feature("收款单模块")
    @allure.story("删除单个收款单")
    @pytest.mark.parametrize('udata', loadyaml(
        'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/fundsmanagement/receipt/delete.yaml'))
    def test_5_delete_customer(self, udata, init_driver):
        '''基础设置-删除收款单用例'''
        self.lp = Fund_Management_receipt(init_driver).delete_receipt(udata['Name'])
        pytest.assume(self.lp == udata['message'])
        # 动态allure标题
        allure.dynamic.title(f'{udata["legend"]}')
        allure.attach(str([f"{udata['message']} 等于 {self.lp}"]), "断言信息", allure.attachment_type.TEXT)  # 保存文本为allure的附件
...
if __name__ == '__main__':
    ...
    pytest.main(
        ['-s', 'D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/test_case/test_case_fundsmanagement/test_case_fundsmanagement_receipt/test_case_fundsmanagement_receipt.py',
