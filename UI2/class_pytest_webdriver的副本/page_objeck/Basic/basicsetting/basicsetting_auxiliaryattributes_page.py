...
class Basic_Setting_auxiliaryattributes(BasePage):
    # 核心元素
    data = loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/auxiliaryattributes/InsertInDatabase.yaml')
    ...
    auxiliaryattributes = (By.ID, data1['AuxiliaryAttributes'])
    ...
    # 下拉框
    AuxiliaryAttributesclassify = (By.CSS_SELECTOR, data1['AuxiliaryAttributesclassify'])
    choixce = (By.XPATH, data1['choixce'])
    ...
    newcustomername2 =(By.XPATH, data1['newcustomername2'])
    savename2 =(By.CSS_SELECTOR, data1['savename2'])
    # 查
    ...
    # 基础资料 新增辅助属性
    def add_auxiliaryattributes(self,auxiliaryattributesName):
        try:
            # a = Login(driver)
            # a.login1()
            # time.sleep(15)
            ...
            # 点击进入辅助属性页面
            self.click(self.auxiliaryattributes)
            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.auxiliaryattributes}点击新增",
            ...
            self.input(self.newcustomername,auxiliaryattributesName)
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.newcustomername}输入客户名",
                      allure.attachment_type.PNG)  # 保存截图为allure的附件

            time.sleep(2)
            self.click(self.AuxiliaryAttributesclassify)
            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.AuxiliaryAttributesclassify}下拉框",
                  allure.attachment_type.PNG)  # 保存截图为allure的附件
            self.click(self.choixce)

            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.AuxiliaryAttributesclassify}点击尺寸",
                  allure.attachment_type.PNG)  # 保存截图为allure的附件
            ...
            # 获取“请求成功”提示弹窗文本
            if auxiliaryattributesName == "名称重复辅助属性":
    ...
    def change_auxiliaryattributes(self,changeCustomerName):
        try:
            # a = Login(driver)
            # a.login1()
            # # 鼠标悬浮到基础设置按钮
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "修改辅助属性")
            ...
            if changeCustomerName =="名称重复辅助属性":
                self.input(self.newcustomername, changeCustomerName)
                allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.newcustomername}输入客户名",
                                  allure.attachment_type.PNG)  # 保存截图为allure的附件
                # 点击保存
                self.click(self.savename2)
                allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.savename2}点击保存",
                              allure.attachment_type.PNG)  # 保存截图为allure的附件
    ...
    # 基础资料 查询客户信息
    def seach_auxiliaryattributes(self,queryName):
        try:
    ...
    # 基础资料 删除单条客户信息
    def delete_auxiliaryattributes(self,deleteName):
        try:
            self.refresh()
            time.sleep(2)
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "UI自动化新增辅助属性001")
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.seachcustomer}输入查询信息",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
            # 点击查询
            self.click(self.seach2)
...
if __name__ == '__main__':
    ...
    lp = Basic_Setting_auxiliaryattributes(driver)
    k = lp.add_auxiliaryattributes("名称重复辅助属性")
    # lp.seach_auxiliaryattributes("单个精准查询")
    # lp.delete_auxiliaryattributes("删除选手")
    # lp.change_auxiliaryattributes("修改辅助属性")
