...
class Basic_Setting_Warhouse(BasePage):
    # 核心元素
    data = loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/warhouse/InsertInDatabase.yaml')
    ...
    # 基础资料 新增仓库
    def add_Warhouse(self,warhouseName):
        try:
            # a = Login(driver)
            # a.login1()
            # # 鼠标悬浮到基础设置按钮
            ...
            # 点击进入仓库页面
            self.click(self.warhouse)
            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.warhouse}点击新增",
            ...
            self.input(self.newcustomername,warhouseName)
    ...
    def change_Warhouse(self,changeCustomerName):
        try:
            # a = Login(driver)
            # a.login1()
            # # 鼠标悬浮到基础设置按钮
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "修改仓库")
    ...
    # 基础资料 查询客户信息
    def seach_Warhouse(self,queryName):
        try:
            self.refresh()
            time.sleep(2)
    ...
    # 基础资料 删除单条客户信息
    def delete_Warhouse(self,deleteName):
        try:
            self.refresh()
            time.sleep(2)
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "UI自动化新增仓库001")
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.seachcustomer}输入查询信息",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
            # 点击查询
            self.click(self.seach2)
...
if __name__ == '__main__':
    ...
    lp = Basic_Setting_Warhouse(driver)
    # k = lp.add_Warhouse("3231")
    # lp.seach_Warhouse("单个精准查询")
    # lp.delete_Warhouse("删除选手")
    # lp.change_Warhouse("名称重复仓库")
