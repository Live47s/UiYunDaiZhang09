...
class Basic_Setting_Account(BasePage):
    # 核心元素
    data = loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/account/InsertInDatabase.yaml')
    ...
    # 查
    # seachcustomer1= (By.CLASS_NAME, data1['seachcustomer1'])
    ...
    # 基础资料 新增客户
    def add_Account(self,customerName):

        try:

            # 点击进入仓库页面
            self.click(self.account)
            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.account}点击新增",
    ...
    def change_Account(self,changeCustomerName):
        try:
        #     a = Login(driver)
        #     a.login1()
        #     # self.refresh()
        ...
        #     driver.implicitly_wait(10)
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "修改账户")
            ...
            allure.attach(self.driver.get_screenshot_as_png(), f"{self.savecustomer}用例失败的截图",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
            self.refresh()
            return "用例执行失败"
    ...
    # 基础资料 查询客户信息
    def seach_Account(self,queryName):
        try:
            # a = Login(driver)
            # a.login1()
            # time.sleep(15)
    ...
    # 基础资料 删除单条客户信息
    def delete_Account(self,deleteName):
        try:
            # a = Login(driver)
            # a.login1()
            self.refresh()
            ...
            # 输入查询客户的名称
            self.input(self.seachcustomer, "UI自动化新增账户001")
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.seachcustomer}输入查询信息",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
            time.sleep(2)
            # 点击查询
...
if __name__ == '__main__':
    ...
    lp = Basic_Setting_Account(driver)
    # k = lp.add_Account("新增账户")
    # lp.change_Account("修改账户数据")
    lp.seach_Account("单个精准查询账户")
    # lp.delete_Account("UI自动化新增账户001")
    driver.quit()