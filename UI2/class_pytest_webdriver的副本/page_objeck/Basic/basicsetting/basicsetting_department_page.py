...
class Basic_Setting_department(BasePage):
    # 核心元素
    data = loadyaml('D:/Pycharm/UiYunDaiZhang/UI2/class_pytest_webdriver的副本/data/basic/BaseData/department/InsertInDatabase.yaml')
    ...
    department = (By.ID, data1['department'])
    ...
    def add_department(self,departmentName):
        try:
            ...
            allure.attach(self.driver.get_screenshot_as_png(), f"点击{self.department}点击新增",
            ...
            self.input(self.newcustomername,departmentName)
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.newcustomername}输入部门名",
                      allure.attachment_type.PNG)  # 保存截图为allure的附件
    ...
    # 基础资料 修改部门信息,先查询待修改数据，再修改
    def change_department(self,changeCustomerName):
        try:
            # a = Login(driver)
            # a.login2()
            # # 鼠标悬浮到基础设置按钮
        ...
        # time.sleep(2)
            ...
            # 输入查询部门的名称
            self.input(self.seachcustomer, "修改部门")
            ...
            allure.attach(self.driver.get_screenshot_as_png(), f"清除{self.newcustomername}清除部门名",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
    ...
    # 基础资料 查询部门信息
    def seach_department(self,queryName):
        try:
    ...
    # 基础资料 删除单条部门信息
    def delete_department(self,deleteName):
        try:
            ...
            # 输入查询部门的名称
            self.input(self.seachcustomer, "UI自动化新增部门001")
            allure.attach(self.driver.get_screenshot_as_png(), f"输入{self.seachcustomer}输入查询信息",
                          allure.attachment_type.PNG)  # 保存截图为allure的附件
            # 点击查询
            self.click(self.seach2)
...
if __name__ == '__main__':
    ...
    lp = Basic_Setting_department(driver)
    # k = lp.add_department("3231")
    # lp.seach_department("单个精准查询部门")
    # lp.delete_department("删除选手")
    # lp.change_department("修改部门")
